    ...
    def __init__(
        ...
        if self.config.model_type == "t5":
            ...
            self.decoder_input_ids = (
                self.tokenizer.encode("<pad> Passage", return_tensors="pt", add_special_tokens=False).to(self.device)
                if self.tokenizer
                else None
            )
            ...
            if self.config.model_type == "t5":
                ...
                if self.num_permutation == 1:
                    ...
                    for _ in range(self.num_permutation):
                        batch_data.append(
                            [random.sample(id_passage, len(id_passage)), random.sample(labels, len(labels))]
                        )
                    ...
                    output = self.tokenizer.batch_decode(
                        output_ids[:, self.decoder_input_ids.shape[1] :], skip_special_tokens=True
                    )
                    ...
                    if len(candidates) == 0:
                        ...
                        most_common_candidates = [
                            candidate for candidate, count in candidate_counts.items() if count == max_count
                        ]
                ...
                output_ids = self.llm.generate(
                    input_ids, do_sample=False, temperature=0.0, top_p=None, max_new_tokens=1
                )[0]

                self.total_completion_tokens += output_ids.shape[0]

                output = (
                    self.tokenizer.decode(output_ids[input_ids.shape[1] :], skip_special_tokens=True).strip().upper()
                )

            elif self.config.model_type == "phi3":

                conversation = [{"role": "user", "content": input_text + " Passage:"}]
